...
| `pre_push_guard.py` | PreToolUse (Bash) | Compatibility shim delegating to `pre_bash_guard.py`. A session caches `settings.json` at startup, so sessions running across the 2026-07-27 rename still invoke the old path. Delete once no pre-rename session is live. |
