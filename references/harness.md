...
- **deny** (hard-fail, unreachable even for hooks): ALL force pushes (`--force`, `-f`, `--force-with-lease`), `git reset --hard *`, `git commit --amend*`, `git rebase -i *`, `git clean -f*`, catastrophic `rm -rf` targets, `mkfs`, `dd if=*`
