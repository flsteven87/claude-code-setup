---
name: latest
description: Pull every source of truth (git, open PRs, Linear, CHANGELOG) up to where it actually is, then rewrite MEMORY.md in place — syncing drift AND refocusing the file on what the user needs as the session opens. Goes deeper than catchup (it both fetches truth AND restructures memory) and replaces the older housekeeping skill. Use when memory has drifted or grown too verbose across many sessions, when returning to a project after time away, before a planning session that leans on memory, when the user types /latest, or when they say '進入最新狀況', '深度更新 memory', '同步最新', 'memory consolidate', 'consolidate memory', 'tidy memory', '清理 memory', 'memory 太雜亂', 'memory 太長', or 'housekeeping'.
---
