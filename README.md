...
| `bin/codex-hygiene` | Blunt recovery for [codex-plugin-cc#277](https://github.com/openai/codex-plugin-cc/issues/277): `pkill -f` every matching Codex companion/app-server process and delete **all** plugin job-state files. No liveness or staleness check — running it kills healthy jobs too |
| `bin/codex-reconcile-phantoms.sh` | Reconciles phantom Codex jobs; also wired as the `UserPromptSubmit` hook |
